...
exports.loginController = async (req, res, next) => {
  try {
    const { email, password, mode = config.tokenModes.normal } = req.body;
    ...
    const allowedModes = Object.values(config.tokenModes);
    const safeMode = allowedModes.includes(mode) ? mode : config.tokenModes.normal;
    ...
    logger.info(`[ user.controller.js ]User with email as ${response.email} has been logged in, with mode ${safeMode}`);
  } catch (e) {
...
exports.getBasicInformationController = async (req, res, next) => {
  try {
    ...
    logger.info(
      `[ user.controller.js ] Basic information for the user having email as ${req.user.email} has been sent.`,
    );
  } catch (e) {
    req.error = { status: 500, message: "An Error occurred!" };
    return next(new Error());
...
exports.updateBasicInformationController = async (req, res, next) => {
  ...
  if (!basicInformation || typeof basicInformation !== "object") {
    req.error = { status: 400, message: "Bad Request" };
    return next(new Error());
  }

  const { website } = basicInformation;
  /*if (objective > 200) {
        req.error = {
            status: 411,
...
exports.updateEducationInformationController = async (req, res, next) => {
  try {
    const { educations: educationInformation } = req.body;
